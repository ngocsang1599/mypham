<?php 
	trait productModel{
		...
		public function model_do_edit(){
			...
			$conn = Connection::getInstance();
			//--------
			$result = $conn->prepare("update tbl_product set name=:name, category_id=:category_id, code=:code,price=:price,saleproduct=:saleproduct, description=:description where id=:id");
			$result->execute(array("name"=>$name,"category_id"=>$category_id, "code"=>$code,"price"=>$price, "saleproduct"=>$saleproduct, "description"=>$description ,"id"=>$id));
			//----------
			//neu user chon anh
		...
		public function model_do_add(){
			...
			$conn = Connection::getInstance();
			//chuan bi cau truy van
			$result = $conn->prepare("insert into tbl_product set name=:name, category_id=:category_id, code=:code,price=:price,saleproduct=:saleproduct, description=:description, img=:img ");
			$result->execute(array("name"=>$name,"category_id"=>$category_id,"code"=>$code,"price"=>$price, "saleproduct"=>$saleproduct, "description"=>$description,"img"=>$img));
		}
